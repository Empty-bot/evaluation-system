...
const NewForm = () => {
  ...
  useEffect(() => {
    const duplicateOptionsError = questions.some(
      (question) =>
        (question.type === "multiple_choice" ||
          question.type === "single_choice") &&
  ...
  const handleAddOption = (questionIndex) => {
    const newQuestions = [...questions];
    newQuestions[questionIndex].possible_answers.push(
      `Option n° ${newQuestions[questionIndex].possible_answers.length + 1}`,
    );
    setQuestions(newQuestions);
  };
  ...
  const handleRemoveOption = (questionIndex, optionIndex) => {
    const newQuestions = [...questions];
    newQuestions[questionIndex].possible_answers = newQuestions[
      questionIndex
    ].possible_answers.filter((_, i) => i !== optionIndex);
    newQuestions[questionIndex].hasError = checkDuplicateOptions(
          ...
          >
            ...
            {(question.type === "multiple_choice" ||
              ...
              <div className="mb-4">
                ...
                {hasChoiceOptionsError &&
                  question.possible_answers.some(
                    (option) => !option || option.trim() === "",
                  ) && (
                    <p className="text-red-500 text-sm mt-1">
