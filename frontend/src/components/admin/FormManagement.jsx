...
const FormManagement = () => {
  ...
  const confirmDeleteForm = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/questionnaires/${formToDelete.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const confirmCloseForm = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/questionnaires/${formToClose.id}/close`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setForms(
        forms.map((f) =>
          f.id === formToClose.id ? { ...f, status: "closed" } : f,
        ),
      );
      setFormToClose(null);
    } catch (err) {
  ...
  const confirmPublishForm = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/questionnaires/${formToPublish.id}/publish`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setForms(
        forms.map((f) =>
          f.id === formToPublish.id ? { ...f, status: "published" } : f,
        ),
      );
    ...
    <div className="space-y-4">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <p className="text-lg mb-4 text-center">
              ...
              <strong>{formToDelete.title}</strong> ? Cette action est{" "}
      ...
      {formToClose && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <p className="text-lg mb-4 text-center">
              Êtes-vous sûr de vouloir <strong>clôturer</strong> le
              questionnaire <strong>{formToClose.title}</strong> ? Une fois
              clôturé, il ne pourra plus être modifié.
            </p>
            <div className="flex justify-end space-x-4">
              <button
      ...
      {formToPublish && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <p className="text-lg mb-4 text-center">
              Êtes-vous sûr de vouloir <strong>publier</strong> le questionnaire{" "}
              <strong>{formToPublish.title}</strong> ? Une fois publié, il ne
